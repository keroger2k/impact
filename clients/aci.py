...
class ACIClient:
    ...
    def login(self):
        ...
        try:
            response = self.session.post(login_url, json=payload, verify=os.getenv("IMPACT_VERIFY_SSL", "true").lower() == "true", timeout=10)
    ...
    def get(self, path, action="GET_DATA"):
        ...
        def _do_get():
            start_time = time.time()
            try:
                response = self.session.get(url, verify=os.getenv("IMPACT_VERIFY_SSL", "true").lower() == "true", timeout=15)
                duration = int((time.time() - start_time) * 1000)
