"""utils/maintenance_report.py — Maintenance Mode Scheduler (SolarWinds AlertSuppression).
...
three server-side steps — parse/validate, resolve node names to Orion Uris,
and schedule one already-resolved node. routers/reports.py owns per-row SSE
progress; keeping each step here (rather than inline in the router) makes
them independently testable and mirrors how utils/bandwidth_report.py
separates SWQL resolution from routers/reports.py's HTTP surface.
...
real write against whatever entity it resolves to.

Resolves to each node's **Uri**, not its NodeID: this report mutes alerts
via `clients.solarwinds.suppress_alerts()` (`Orion.AlertSuppression`), which
is keyed by Uri — see that function's docstring for the full story of why
this isn't the classic `Orion.Nodes/Unmanage` verb (used by an earlier
version of this module) and why muted nodes still don't show up in
SolarWinds' own "Manage Maintenance Schedules" screen.
"""
...
def resolve_node_uris(names: list[str], username: str, password: str) -> dict[str, dict]:
    ...
    for name in distinct:
        ...
        if not matches:
            ...
            result[name.lower()] = {"uri": matches[0]["Uri"]}
    return result
