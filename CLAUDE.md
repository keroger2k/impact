...
- **Reports** (`/reports`) — on-demand compliance deliverables. Currently just **CDRL49** (daily WAN response-time for AT&T EIS + SOHO Cisco routers, Deliverable 65): the user picks a date and which sub-reports to include, the app runs live SWQL queries against **SolarWinds Orion** (`clients/solarwinds.py`, read-only — the only exposed call is `query()`, a SWQL SELECT) and streams back a generated landscape PDF (`utils/cdrl49_report.py`, via `reportlab`), optionally zipped with the underlying CSVs. Nothing is cached — a CDRL is a point-in-time deliverable, not a dataset that benefits from staleness, so every click re-queries SolarWinds fresh (same "live verify over local logic" posture as the firewall Policy Tester). Auth reuses the logged-in user's own AD creds (like F5), NTLM-formatted as `DOMAIN\username` — no dedicated service account.
...
- `routers/reports.py` — `/api/reports/`: `POST /cdrl49/generate` (form: `report_date` YYYY-MM-DD default yesterday, `include_eis`/`include_soho`/`include_csv` bools) runs live SolarWinds SWQL queries and returns the generated PDF (or a `.zip` with the PDF + per-report CSVs when `include_csv`). Synchronous, not SSE — it's two bounded queries, not a per-device fan-out. Nothing cached; see the Reports entry above.
