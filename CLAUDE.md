...
- `clients/solarwinds.py` — **read-only** SolarWinds Orion SWIS client. One function, `query(swql)`, POSTs a SWQL SELECT to the Information Service's JSON query endpoint and returns the result rows — no other verb is exposed. Dedicated `SOLARWINDS_*` service-account creds, not the shared AD creds. Backs the `/reports` CDRL49 report (`utils/cdrl49_report.py` owns the SWQL text + PDF building; `routers/reports.py` owns the HTTP surface).
...
- `SOLARWINDS_URL`, `SOLARWINDS_USERNAME`, `SOLARWINDS_PASSWORD` — SolarWinds Orion, used only by the `/reports` CDRL49 report. Dedicated creds, not the shared AD ones. Optional: `SOLARWINDS_PORT` (default `17774`), `SOLARWINDS_TIMEOUT` (default `180`s)
