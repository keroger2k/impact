...
- `routers/firewall.py` — `/api/firewall/`: device-groups, policy lookup, **policy tester** (`GET /policy-tester` UI partial, `GET /zones?device_group=` datalist autocomplete, `POST /policy-test` multi-flow form-encoded evaluation against the cached rulebase — zone-aware, intrazone/interzone default verdicts, App-ID not evaluated), policies by DG, rules export, interfaces (list/search), devices, templates
