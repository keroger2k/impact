...
- `scripts/solarwinds_discover_site_properties.py` — read-only SWQL reconnaissance probe for adding a "Site Information" section (Site Code, Airport Code, Category, Building, Local POC, FRM, Circuit Size & Provider, etc.) to the Bandwidth Utilization report. Queries, for one sample router: standard `Orion.Nodes` free-text columns (Location/Contact/Description/Comments — common dumping grounds for this kind of metadata), the full list of custom-property definitions targeting nodes (`Orion.CustomProperty WHERE TargetEntity = 'Orion.Nodes'` — the authoritative "what fields exist" answer), and that node's actual custom-property values (`Orion.NodesCustomProperties`). Not part of the app's runtime — a one-time discovery step before wiring real field names into `utils/bandwidth_report.py`. `.venv/bin/python -m scripts.solarwinds_discover_site_properties` (needs `SW_DISCOVER_NODE`, a router Caption/substring known to have this info filled in, in `.env`).
