...
def mock_aci():
    ...
    # Corrected DNs to match ACI's tree structure and the logic in routers/aci.py
    aci.get_node_interfaces.return_value = {
        "imdata": [
            ...
            {"vpcRsVpcConf": {"attributes": {"dn": "topology/pod-1/node-101/sys/vpc/inst/dom-1/if-[vPC-10]/rsvpcConf", "tDn": "topology/pod-1/node-101/sys/aggr-[po10]", "parentSKey": "10"}}}
        ]
    }
    return aci
...
async def test_node_interfaces_join_logic(mock_aci):
    with patch("routers.aci._get_aci_async", return_value=mock_aci), \
        ...
        # We need to bypass the _cached mechanism if we want to test the normalization logic directly,
        # but here we are calling the router endpoint which uses it.
        # Actually, in routers/aci.py, it uses run_in_executor to call aci.get_node_interfaces.
