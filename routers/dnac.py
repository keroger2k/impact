...
async def list_devices(
    ...
    # ── Merge Nexus devices ──────────────────────────────────────────────────
    nexus_inventory = cache.get("nexus_inventory") or []
    if nexus_inventory:
        nexus_filtered = nexus_inventory
        if hostname:
            nexus_filtered = [d for d in nexus_filtered if hostname.lower() in (d.get("hostname") or "").lower()]
        if ip:
            nexus_filtered = [d for d in nexus_filtered if ip in (d.get("managementIpAddress") or "")]
        if platform:
            nexus_filtered = [d for d in nexus_filtered if platform.lower() in (d.get("platformId") or "").lower()]
        if reachability:
            if reachability.lower() == "reachable":
                nexus_filtered = [d for d in nexus_filtered if d.get("reachabilityStatus") == "Reachable"]
            elif reachability.lower() == "unreachable":
                nexus_filtered = [d for d in nexus_filtered if d.get("reachabilityStatus") != "Reachable"]
        # Nexus devices don't have sites in this implementation
        if site:
            nexus_filtered = []

        filtered = list(filtered) + nexus_filtered
...
async def get_device_detail_partial(
    ...
    if not device:
        # Check Nexus
        nexus_devices = cache.get("nexus_inventory") or []
        device = next((d for d in nexus_devices if d.get("id") == device_id), None)
        if device:
            from templates_module import templates
...
async def get_device_config(
    ...
    # 1. Check Nexus
    nexus_devices = cache.get("nexus_inventory") or []
    nexus_device = next((d for d in nexus_devices if d.get("id") == device_id), None)
    if nexus_device:
        ip = nexus_device.get("managementIpAddress")
        config = cache.get(f"nexus_config_{ip}")
        if not config:
            raise HTTPException(404, "Config not found in cache for Nexus device. Refresh Nexus cache.")
...
async def ip_lookup_handler(ip: str, session: SessionEntry = Depends(require_auth)):
    ...
    nexus_interfaces_cache = cache.get("nexus_interfaces") or {}
    nexus_hits = []
    for n_ip, ifaces in nexus_interfaces_cache.items():
        for iface in ifaces:
            iface_ip_raw = iface.get("ipv4_address", "N/A")
            iface_ip = iface_ip_raw.split("/")[0] if "/" in iface_ip_raw else iface_ip_raw
